...
typedef struct {
    ...
    uint32_t PCROPConfig;      /*!< Configuration of the PCROP (used for
                                  ...
                                  combination of @ref FLASH_OB_PCROP_ZONE      and
                                  @ref      FLASH_OB_PCROP_RDP. Note that once set,
                                  Pcrop      erase      on RDP level 1 regression
                                  (PCROP_RDP      bit) can not      be reset. It will
                                  be      reset by mass      erase */
    uint32_t PCROP1AStartAddr; /*!< PCROP Start address (used for
                                  OPTIONBYTE_PCROP). It represents first address
                                  of start block to protect. Make sure this
    ...
    uint32_t PCROP1BStartAddr; /*!< PCROP Start address (used for
                                  OPTIONBYTE_PCROP). It represents first address
                                  of start block to protect. Make sure this
                                  parameter is multiple of PCROP granularity:
                                  512 Bytes.*/
...
#    if defined(FLASH_DBANK_SUPPORT)
    uint32_t PCROP2AStartAddr; /*!< PCROP Start address (used for
                                  OPTIONBYTE_PCROP). It represents first address
                                  of start block to protect. Make sure this
                                  parameter is multiple of PCROP granularity:
                                  512 Bytes.*/
    uint32_t PCROP2AEndAddr; /*!< PCROP End address (used for OPTIONBYTE_PCROP).
                                ...
                                protect. Make sure this parameter is multiple of
                                PCROP granularity: 512 Bytes.*/
    uint32_t PCROP2BStartAddr; /*!< PCROP Start address (used for
                                  OPTIONBYTE_PCROP). It represents first address
                                  of start block to protect. Make sure this
                                  parameter is multiple of PCROP granularity:
                                  512 Bytes.*/
...
#    if defined(FLASH_DBANK_SUPPORT)
    uint32_t SecSize2; /*!< This parameter defines securable memory area width
                          in number of pages starting from 2nd Bank start
                          address. This parameter must be a value between [0]
                          and [FLASH_PAGE_NB], [0] meaning no secure area
                          defined, [1] meaning first page only protected, etc...
                        */
...
#define FLASH_FLAG_EOP                                                         \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_EOP_Pos) /*!< FLASH End of operation flag */
#define FLASH_FLAG_OPERR                                                       \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_OPERR_Pos) /*!< FLASH Operation error flag */
#define FLASH_FLAG_PROGERR                                                     \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_PROGERR_Pos) /*!< FLASH Programming error flag */
#define FLASH_FLAG_WRPERR                                                      \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_WRPERR_Pos) /*!< FLASH Write protection error flag */
#define FLASH_FLAG_PGAERR                                                      \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_PGAERR_Pos) /*!< FLASH Programming alignment error flag */
#define FLASH_FLAG_SIZERR                                                      \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_SIZERR_Pos) /*!< FLASH Size error flag  */
#define FLASH_FLAG_PGSERR                                                      \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_PGSERR_Pos) /*!< FLASH Programming sequence error flag */
#define FLASH_FLAG_MISERR                                                      \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_MISERR_Pos) /*!< FLASH Fast programming data miss error flag   \
                             */
#define FLASH_FLAG_FASTERR                                                     \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_FASTERR_Pos) /*!< FLASH Fast programming error flag */
#if defined(FLASH_PCROP_SUPPORT)
#    define FLASH_FLAG_RDERR                                                   \
        ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                              \
         | FLASH_SR_RDERR_Pos) /*!< FLASH PCROP read error flag */
#endif                         /* FLASH_PCROP_SUPPORT */
#define FLASH_FLAG_OPTVERR                                                     \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_OPTVERR_Pos) /*!< FLASH Option validity error flag */
#define FLASH_FLAG_BSY1                                                        \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_BSY1_Pos) /*!< FLASH Operation Busy flag for Bank 1 */
#if defined(FLASH_DBANK_SUPPORT)
#    define FLASH_FLAG_BSY2                                                    \
        ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                              \
         | FLASH_SR_BSY2_Pos) /*!< FLASH Operation Busy flag for Bank 2 */
#endif                        /* FLASH_DBANK_SUPPORT */
...
#define FLASH_FLAG_CFGBSY                                                      \
    ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_SR_CFGBSY_Pos) /*!< FLASH Configuration Busy flag */
#if defined(FLASH_DBANK_SUPPORT)
#    define FLASH_FLAG_PESD                                                    \
        ((FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)                              \
         | FLASH_SR_PESD_Pos) /*!< FLASH Programming/erase operation suspended \
                               */
#endif                        /* FLASH_DBANK_SUPPORT */
#define FLASH_FLAG_ECCC1                                                       \
    ((FLASH_FLAG_ECCR1_ID << FLASH_FLAG_REG_POS)                               \
     | FLASH_ECCR_ECCC_Pos) /*!< FLASH ECC correction on bank 1 */
#define FLASH_FLAG_ECCD1                                                       \
    ((FLASH_FLAG_ECCR1_ID << FLASH_FLAG_REG_POS)                               \
     | FLASH_ECCR_ECCD_Pos) /*!< FLASH ECC detection on bank 1 */
#if defined(FLASH_DBANK_SUPPORT)
#    define FLASH_FLAG_ECCC2                                                   \
        ((FLASH_FLAG_ECCR2_ID << FLASH_FLAG_REG_POS)                           \
         | FLASH_ECC2R_ECCC_Pos) /*!< FLASH ECC correction on bank 2 */
#    define FLASH_FLAG_ECCD2                                                   \
        ((FLASH_FLAG_ECCR2_ID << FLASH_FLAG_REG_POS)                           \
         | FLASH_ECC2R_ECCD_Pos) /*!< FLASH ECC detection on bank 2 */
#endif                           /* FLASH_DBANK_SUPPORT */
#define FLASH_FLAG_ECCC                                                        \
    FLASH_FLAG_ECCC1 /*!< FLASH ECC correction - legacy name for single bank   \
                      */
#define FLASH_FLAG_ECCD                                                        \
    FLASH_FLAG_ECCD1 /*!< FLASH ECC detection - legacy name for single bank */
/**
 * @}
 */
 ...
 */
#define FLASH_IT_EOP                                                           \
    ((FLASH_FLAG_CR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_CR_EOPIE_Pos) /*!< End of FLASH Operation Interrupt source */
#define FLASH_IT_OPERR                                                         \
    ((FLASH_FLAG_CR_ID << FLASH_FLAG_REG_POS)                                  \
     | FLASH_CR_ERRIE_Pos) /*!< Error Interrupt source */
#if defined(FLASH_PCROP_SUPPORT)
#    define FLASH_IT_RDERR                                                     \
        ((FLASH_FLAG_CR_ID << FLASH_FLAG_REG_POS)                              \
         | FLASH_CR_RDERRIE_Pos) /*!< PCROP Read Error Interrupt source*/
#endif                           /* FLASH_PCROP_SUPPORT */
...
#    define FLASH_IT_ECCC2                                                     \
        ((FLASH_FLAG_ECCR2_ID << FLASH_FLAG_REG_POS)                           \
         | FLASH_ECC2R_ECCCIE_Pos) /*!< ECC Correction on Bank 2 Interrupt     \
                                      source */
#endif                             /* FLASH_DBANK_SUPPORT */
#define FLASH_IT_ECCC                                                          \
    FLASH_IT_ECCC1 /*!< ECC Correction - legacy name for single bank */
/**
 * @}
 */
...
#    define OPTIONBYTE_PCROP                                                   \
        0x00000008U /*!< PCROP option byte configuration                       \
                     */
#endif              /* FLASH_PCROP_SUPPORT */
#if defined(FLASH_SECURABLE_MEMORY_SUPPORT)
...
#else
#    define OPTIONBYTE_ALL                                                     \
        (OPTIONBYTE_WRP | OPTIONBYTE_RDP                                       \
         | OPTIONBYTE_USER) /*!< All option byte configuration */
...
#    define OB_USER_BOR_LEV                                                    \
        (FLASH_OPTR_BORF_LEV | FLASH_OPTR_BORR_LEV) /*!< BOR reset Level */
#endif                                              /* PWR_BOR_SUPPORT */
#define OB_USER_nRST_STOP                                                      \
    FLASH_OPTR_nRST_STOP /*!< Reset generated when entering the stop mode */
...
#    define OB_USER_nRST_SHDW                                                  \
        FLASH_OPTR_nRST_SHDW /*!< Reset generated when entering the shutdown   \
                                mode */
#endif                       /* PWR_SHDW_SUPPORT */
#define OB_USER_IWDG_SW                                                        \
    FLASH_OPTR_IWDG_SW /*!< Independent watchdog selection */
...
#if defined(FLASH_DBANK_SUPPORT)
#    define OB_USER_BANK_SWAP                                                  \
        FLASH_OPTR_nSWAP_BANK /*!< Swap bank memory addresses */
#    define OB_USER_DUAL_BANK                                                  \
        FLASH_OPTR_DUAL_BANK /*!< Select single or dual bank (depending of     \
                                device memory size) */
#endif                       /* FLASH_DBANK_SUPPORT */
#define OB_USER_RAM_PARITY_CHECK                                               \
...
#    define OB_USER_NRST_MODE                                                  \
        FLASH_OPTR_NRST_MODE /*!< Reset pin configuration */
#endif                       /* GPIO_NRST_CONFIG_SUPPORT */
#if defined(FLASH_OPTR_IRHEN)
#    define OB_USER_INPUT_RESET_HOLDER                                         \
        FLASH_OPTR_IRHEN /*!< Internal reset holder enable */
#endif                   /* FLASH_OPTR_IRHEN */

#if defined(FLASH_DBANK_SUPPORT)
#    if defined(PWR_BOR_SUPPORT) && defined(PWR_SHDW_SUPPORT)                  \
        && defined(GPIO_NRST_CONFIG_SUPPORT)
#        define OB_USER_ALL                                                    \
            (OB_USER_BOR_EN | OB_USER_BOR_LEV | OB_USER_nRST_STOP              \
             ...
             | OB_USER_nBOOT0 | OB_USER_NRST_MODE                              \
             | OB_USER_INPUT_RESET_HOLDER) /*!< all option bits */
#    else
#        define OB_USER_ALL                                                    \
            (OB_USER_nRST_STOP | OB_USER_nRST_STDBY | OB_USER_IWDG_SW          \
             | OB_USER_IWDG_STOP | OB_USER_IWDG_STDBY | OB_USER_WWDG_SW        \
             | OB_USER_BANK_SWAP | OB_USER_DUAL_BANK                           \
             | OB_USER_RAM_PARITY_CHECK | OB_USER_nBOOT_SEL | OB_USER_nBOOT1   \
...
#    define OB_BOR_ENABLE                                                      \
        FLASH_OPTR_BOR_EN /*!< Use option byte to define BOR thresholds */
/**
 * @}
 */
 ...
 * @{
 */
#    define OB_BOR_LEVEL_FALLING_0                                             \
        0x00000000U /*!< BOR falling level 1 with threshold around 2.0V */
#    define OB_BOR_LEVEL_FALLING_1                                             \
        FLASH_OPTR_BORF_LEV_0 /*!< BOR falling level 2 with threshold          \
                                 around 2.2V */
#    define OB_BOR_LEVEL_FALLING_2                                             \
        FLASH_OPTR_BORF_LEV_1 /*!< BOR falling level 3 with threshold          \
                                 around 2.5V */
#    define OB_BOR_LEVEL_FALLING_3                                             \
        (FLASH_OPTR_BORF_LEV_0 | FLASH_OPTR_BORF_LEV_1) /*!< BOR falling level \
                                                           4 with threshold    \
                                                           around 2.8V */
#    define OB_BOR_LEVEL_RISING_0                                              \
        0x00000000U /*!< BOR rising level 1 with threshold around 2.1V */
#    define OB_BOR_LEVEL_RISING_1                                              \
        FLASH_OPTR_BORR_LEV_0 /*!< BOR rising level 2 with threshold           \
                                 around 2.3V */
#    define OB_BOR_LEVEL_RISING_2                                              \
        FLASH_OPTR_BORR_LEV_1 /*!< BOR rising level 3 with threshold           \
                                 around 2.6V */
#    define OB_BOR_LEVEL_RISING_3                                              \
        (FLASH_OPTR_BORR_LEV_0 | FLASH_OPTR_BORR_LEV_1) /*!< BOR rising level  \
                                                           4 with threshold    \
                                                           around 2.9V */
/**
 * @}
...
#    define OB_USER_DUALBANK_SWAP_DISABLE                                      \
        FLASH_OPTR_nSWAP_BANK /*!< Disable bank swap */
/**
 * @}
 */
...
#    define OB_USER_DUALBANK_ENABLE                                            \
        FLASH_OPTR_DUAL_BANK /*!< Enable dual bank */
/**
 * @}
 */
#endif /* FLASH_DBANK_SUPPORT */
...
#define OB_SRAM_PARITY_DISABLE                                                 \
    FLASH_OPTR_RAM_PARITY_CHECK /*!< Sram parity disable */
/**
 * @}
 */
...
#define OB_BOOT0_FROM_PIN                                                      \
    0x00000000U /*!< BOOT0 signal is defined by PA14/BOOT0 pin value */
#define OB_BOOT0_FROM_OB                                                       \
    FLASH_OPTR_nBOOT_SEL /*!< BOOT0 signal is defined by nBOOT0 option bit */
/**
 * @}
 */
...
#    define OB_RESET_MODE_GPIO                                                 \
        FLASH_OPTR_NRST_MODE_1 /*!< Reset pin is in GPIO mode mode only */
...
#    define OB_IRH_DISABLE                                                     \
        FLASH_OPTR_IRHEN /*!< Internal Reset handler disable */
/**
 * @}
 */
#endif /* FLASH_OPTR_IRHEN */
...
#    define OB_PCROP_RDP_ERASE                                                 \
        FLASH_PCROP1AER_PCROP_RDP /*!< PCROP area is erased when the RDP level \
                                     is decreased from Level 1 to Level 0      \
                                     (full mass erase). Once this bit is set   \
                                     only, it will be reset by mass erase */
/**
 * @}
 */
#endif /* FLASH_PCROP_SUPPORT */
...
#    define OB_BOOT_ENTRY_FORCED_FLASH                                         \
        FLASH_SECR_BOOT_LOCK /*!< Boot entry is forced to Flash or System      \
                                Flash */
/**
 * @}
 */
...
#    define __HAL_FLASH_ENABLE_IT(__INTERRUPT__)                               \
        do {                                                                   \
            if (((__INTERRUPT__) & (FLASH_FLAG_CR_ID << FLASH_FLAG_REG_POS))   \
                != 0U) {                                                       \
                SET_BIT(FLASH->CR, (1uL << ((__INTERRUPT__)&0x1Fu)));          \
            } else if (((__INTERRUPT__)                                        \
                        & (FLASH_FLAG_ECCR1_ID << FLASH_FLAG_REG_POS))         \
                       != 0U) {                                                \
...
#    define __HAL_FLASH_GET_FLAG(__FLAG__)                                     \
        ((((__FLAG__) & (FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)) != 0U)       \
             ...
             : ((((__FLAG__) & (FLASH_FLAG_ECCR1_ID << FLASH_FLAG_REG_POS))    \
                 != 0U)                                                        \
                    ? (READ_BIT(FLASH->ECCR, (1uL << ((__FLAG__)&0x1Fu)))      \
                       != 0x00u)                                               \
                    : (READ_BIT(FLASH->ECC2R, (1uL << ((__FLAG__)&0x1Fu)))     \
                       != 0x00u)))
#else
#    define __HAL_FLASH_GET_FLAG(__FLAG__)                                     \
        ((((__FLAG__) & (FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS)) != 0U)       \
...
#    define __HAL_FLASH_CLEAR_FLAG(__FLAG__)                                   \
        do {                                                                   \
            if (((__FLAG__) & (FLASH_FLAG_SR_ID << FLASH_FLAG_REG_POS))        \
                != 0U) {                                                       \
...
#    define OB_DUAL_BANK_BASE                                                  \
        (FLASH_R_BASE + 0x20U) /*!< Not use cmsis FLASH alias to avoid iar     \
                                  warning about volatile reading sequence */
#    define FLASH_SALES_TYPE_Pos (24U)
#    define FLASH_SALES_TYPE                                                   \
        (0x3UL << FLASH_SALES_TYPE_Pos) /*!< 0x000001E0                        \
                                         */
#    define FLASH_SALES_TYPE_0                                                 \
        (0x1UL << FLASH_SALES_TYPE_Pos) /*!< 0x01000000 */
#    define FLASH_SALES_TYPE_1                                                 \
        (0x2UL << FLASH_SALES_TYPE_Pos) /*!< 0x02000000 */
#    define FLASH_SALES_VALUE                                                  \
        ((*((uint32_t *)PACKAGE_BASE)) & (FLASH_SALES_TYPE))
#    define OB_DUAL_BANK_VALUE                                                 \
        ((*((uint32_t *)OB_DUAL_BANK_BASE)) & (FLASH_OPTR_DUAL_BANK))
#    define FLASH_BANK_NB                                                      \
        (((FLASH_SALES_VALUE == 0U)                                            \
          || ((FLASH_SALES_VALUE == FLASH_SALES_TYPE_0)                        \
              && (OB_DUAL_BANK_VALUE == 0U)))                                  \
             ? 1U                                                              \
             : 2U)
#    define FLASH_BANK_SIZE                                                    \
        ((FLASH_BANK_NB == 1U) ? (FLASH_SIZE)                                  \
                               : (FLASH_SIZE >> 1U)) /*!< FLASH Bank Size.     \
                                                        Divided by 2 if 2      \
                                                        Banks */
...
#define FLASH_PAGE_NB                                                          \
    (FLASH_BANK_SIZE / FLASH_PAGE_SIZE) /* Number of pages per bank */
...
#    define FLASH_SR_ERRORS                                                    \
        (FLASH_SR_OPERR | FLASH_SR_PROGERR | FLASH_SR_WRPERR | FLASH_SR_PGAERR \
         | FLASH_SR_SIZERR | FLASH_SR_PGSERR | FLASH_SR_MISERR                 \
         | FLASH_SR_FASTERR | FLASH_SR_OPTVERR) /*!< All SR error flags */
#endif                                          /* FLASH_PCROP_SUPPORT */
...
#    define IS_FLASH_BANK(__BANK__)                                            \
        ((FLASH_BANK_NB == 2U)                                                 \
             ? (((__BANK__) == FLASH_BANK_1) || ((__BANK__) == FLASH_BANK_2)   \
...
#define IS_FLASH_TYPEERASE(__VALUE__)                                          \
...
#    define IS_OB_WRPAREA(__VALUE__)                                           \
        ((FLASH_BANK_NB == 2U) ? (((__VALUE__) == OB_WRPAREA_ZONE_A)           \
                                  || ((__VALUE__) == OB_WRPAREA_ZONE_B)        \
                                  || ((__VALUE__) == OB_WRPAREA_ZONE2_A)       \
                                  || ((__VALUE__) == OB_WRPAREA_ZONE2_B))      \
...
#define IS_OB_RDP_LEVEL(__LEVEL__)                                             \
    (((__LEVEL__) == OB_RDP_LEVEL_0) || ((__LEVEL__) == OB_RDP_LEVEL_1)        \
     || ((__LEVEL__) == OB_RDP_LEVEL_2))

#define IS_OB_USER_TYPE(__TYPE__)                                              \
    ((((__TYPE__)&OB_USER_ALL) != 0x00U)                                       \
     && (((__TYPE__) & ~OB_USER_ALL) == 0x00U))

#define IS_OB_USER_CONFIG(__TYPE__, __CONFIG__)                                \
    ((~(__TYPE__) & (__CONFIG__)) == 0x00U)
...
#        define IS_OB_PCROP_CONFIG(__CONFIG__)                                 \
            ((FLASH_BANK_NB == 2U)                                             \
                 ? (((__CONFIG__)                                              \
                     & ~(OB_PCROP_ZONE_A | OB_PCROP_ZONE_B | OB_PCROP_ZONE2_A  \
                         | OB_PCROP_ZONE2_B | OB_PCROP_RDP_ERASE))             \
                    == 0x00U)                                                  \
                 : (((__CONFIG__)                                              \
                     & ~(OB_PCROP_ZONE_A | OB_PCROP_ZONE_B                     \
                         | OB_PCROP_RDP_ERASE))                                \
                    == 0x00U))
#    else
#        define IS_OB_PCROP_CONFIG(__CONFIG__)                                 \
            (((__CONFIG__)                                                     \
              & ~(OB_PCROP_ZONE_A | OB_PCROP_ZONE_B | OB_PCROP_RDP_ERASE))     \
             == 0x00U)
#    endif /* FLASH_DBANK_SUPPORT */
#endif     /* FLASH_PCROP_SUPPORT */
...
#    define IS_OB_SEC_BOOT_LOCK(__VALUE__)                                     \
        (((__VALUE__) == OB_BOOT_ENTRY_FORCED_NONE)                            \
