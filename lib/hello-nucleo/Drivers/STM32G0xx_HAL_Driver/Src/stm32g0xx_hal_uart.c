...
#    define LPUART_BRR_MIN                                                     \
        0x00000300U /* LPUART BRR minimum authorized value                     \
                     */
#    define LPUART_BRR_MAX                                                     \
        0x000FFFFFU /* LPUART BRR maximum authorized value                     \
                     */
 ...
 */
HAL_StatusTypeDef
HAL_HalfDuplex_Init(UART_HandleTypeDef *huart)
{
    /* Check the UART handle allocation */
    if (huart == NULL) {
        return HAL_ERROR;
 ...
 */
HAL_StatusTypeDef
HAL_LIN_Init(UART_HandleTypeDef *huart, uint32_t BreakDetectLength)
{
    /* Check the UART handle allocation */
    if (huart == NULL) {
        return HAL_ERROR;
 ...
 */
HAL_StatusTypeDef
HAL_MultiProcessor_Init(UART_HandleTypeDef *huart, uint8_t Address,
                            ...
                            HAL_UART_CallbackIDTypeDef CallbackID)
{
    ...
    if (HAL_UART_STATE_READY == huart->gState) {
        ...
        case HAL_UART_MSPINIT_CB_ID:
            huart->MspInitCallback = HAL_UART_MspInit; /* Legacy weak
                                                          MspInitCallback */
            break;
 ...
 */
HAL_StatusTypeDef
HAL_UART_Transmit(UART_HandleTypeDef *huart, const uint8_t *pData,
 ...
 */
HAL_StatusTypeDef
HAL_UART_Transmit_DMA(UART_HandleTypeDef *huart, const uint8_t *pData,
                      uint16_t Size)
{
    /* Check that a Tx process is not already ongoing */
    if (huart->gState == HAL_UART_STATE_READY) {
        ...
        if (huart->hdmatx != NULL) {
            ...
            if (HAL_DMA_Start_IT(huart->hdmatx, (uint32_t)huart->pTxBuffPtr,
                                 (uint32_t)&huart->Instance->TDR, Size)
                != HAL_OK) {
 ...
 */
void
HAL_UART_ReceiverTimeout_Config(UART_HandleTypeDef *huart,
 ...
 */
HAL_StatusTypeDef
HAL_UART_EnableReceiverTimeout(UART_HandleTypeDef *huart)
{
    if (!(IS_LPUART_INSTANCE(huart->Instance))) {
        if (huart->gState == HAL_UART_STATE_READY) {
            /* Process Locked */
 ...
 */
HAL_StatusTypeDef
HAL_UART_DisableReceiverTimeout(UART_HandleTypeDef *huart)
{
    if (!(IS_LPUART_INSTANCE(huart->Instance))) {
        if (huart->gState == HAL_UART_STATE_READY) {
            /* Process Locked */
 ...
 */
HAL_StatusTypeDef
HAL_MultiProcessor_EnableMuteMode(UART_HandleTypeDef *huart)
{
    __HAL_LOCK(huart);
 ...
 */
HAL_StatusTypeDef
HAL_MultiProcessor_DisableMuteMode(UART_HandleTypeDef *huart)
{
    __HAL_LOCK(huart);
 ...
 */
void
HAL_MultiProcessor_EnterMuteMode(UART_HandleTypeDef *huart)
{
 ...
 */
uint32_t
HAL_UART_GetError(const UART_HandleTypeDef *huart)
{
...
UART_InitCallbacksToDefault(UART_HandleTypeDef *huart)
{
    ...
    huart->AbortReceiveCpltCallback =
        HAL_UART_AbortReceiveCpltCallback;             /* Legacy weak
                                                          AbortReceiveCpltCallback  */
    huart->WakeupCallback = HAL_UARTEx_WakeupCallback; /* Legacy weak
                                                          WakeupCallback */
    huart->RxFifoFullCallback =
        HAL_UARTEx_RxFifoFullCallback; /* Legacy weak RxFifoFullCallback */
    huart->TxFifoEmptyCallback =
 ...
 */
HAL_StatusTypeDef
UART_SetConfig(UART_HandleTypeDef *huart)
{
    uint32_t                tmpreg;
    uint16_t                brrtemp;
    ...
    if (UART_INSTANCE_LOWPOWER(huart)) {
        ...
        if (pclk != 0U) {
            ...
            lpuart_ker_ck_pres =
                (pclk / UARTPrescTable[huart->Init.ClockPrescaler]);
 ...
 */
void
UART_AdvFeatureConfig(UART_HandleTypeDef *huart)
{
 ...
 */
HAL_StatusTypeDef
UART_CheckIdleState(UART_HandleTypeDef *huart)
{
    uint32_t tickstart;
    ...
    if ((huart->Instance->CR1 & USART_CR1_TE) == USART_CR1_TE) {
        ...
        if (UART_WaitOnFlagUntilTimeout(huart, USART_ISR_TEACK, RESET,
                                        tickstart, HAL_UART_TIMEOUT_VALUE)
            != HAL_OK) {
            /* Timeout occurred */
            return HAL_TIMEOUT;
        }
 ...
 */
HAL_StatusTypeDef
UART_WaitOnFlagUntilTimeout(UART_HandleTypeDef *huart, uint32_t Flag,
 ...
 */
HAL_StatusTypeDef
UART_Start_Receive_IT(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
 ...
 */
HAL_StatusTypeDef
UART_Start_Receive_DMA(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size)
{
    huart->pRxBuffPtr = pData;
    huart->RxXferSize = Size;
    ...
    if (huart->hdmarx != NULL) {
        ...
        if (HAL_DMA_Start_IT(huart->hdmarx, (uint32_t)&huart->Instance->RDR,
                             (uint32_t)huart->pRxBuffPtr, Size)
            != HAL_OK) {
            /* Set error code to DMA */
            huart->ErrorCode = HAL_UART_ERROR_DMA;
 ...
 */
static void
UART_EndTxTransfer(UART_HandleTypeDef *huart)
{
 ...
 */
static void
UART_EndRxTransfer(UART_HandleTypeDef *huart)
{
    /* Disable RXNE, PE and ERR (Frame error, noise error, overrun error)
     * interrupts */
    ATOMIC_CLEAR_BIT(huart->Instance->CR1,
 ...
 */
static void
UART_DMARxHalfCplt(DMA_HandleTypeDef *hdma)
{
    UART_HandleTypeDef *huart = (UART_HandleTypeDef *)(hdma->Parent);
 ...
 */
static void
UART_DMAAbortOnError(DMA_HandleTypeDef *hdma)
{
    UART_HandleTypeDef *huart = (UART_HandleTypeDef *)(hdma->Parent);
    huart->RxXferCount        = 0U;
    huart->TxXferCount        = 0U;
 ...
 */
static void
UART_DMATxAbortCallback(DMA_HandleTypeDef *hdma)
{
    UART_HandleTypeDef *huart = (UART_HandleTypeDef *)(hdma->Parent);
 ...
 */
static void
UART_DMARxAbortCallback(DMA_HandleTypeDef *hdma)
{
    UART_HandleTypeDef *huart = (UART_HandleTypeDef *)(hdma->Parent);
 ...
 */
static void
UART_DMATxOnlyAbortCallback(DMA_HandleTypeDef *hdma)
{
    UART_HandleTypeDef *huart = (UART_HandleTypeDef *)(hdma->Parent);
 ...
 */
static void
UART_DMARxOnlyAbortCallback(DMA_HandleTypeDef *hdma)
{
 ...
 */
static void
UART_TxISR_8BIT(UART_HandleTypeDef *huart)
{
    ...
    if (huart->gState == HAL_UART_STATE_BUSY_TX) {
        if (huart->TxXferCount == 0U) {
            ...
            huart->Instance->TDR =
                (uint8_t)(*huart->pTxBuffPtr & (uint8_t)0xFF);
            huart->pTxBuffPtr++;
            huart->TxXferCount--;
        }
    }
 ...
 */
static void
UART_TxISR_16BIT(UART_HandleTypeDef *huart)
{
    const uint16_t *tmp;
 ...
 */
static void
UART_TxISR_8BIT_FIFOEN(UART_HandleTypeDef *huart)
{
    uint16_t nb_tx_data;
 ...
 */
static void
UART_TxISR_16BIT_FIFOEN(UART_HandleTypeDef *huart)
{
    const uint16_t *tmp;
    uint16_t        nb_tx_data;
 ...
 */
static void
UART_RxISR_8BIT(UART_HandleTypeDef *huart)
{
 ...
 */
static void
UART_RxISR_16BIT(UART_HandleTypeDef *huart)
{
    uint16_t *tmp;
    uint16_t  uhMask = huart->Mask;
    uint16_t  uhdata;
 ...
 */
static void
UART_RxISR_8BIT_FIFOEN(UART_HandleTypeDef *huart)
{
    uint16_t uhMask = huart->Mask;
    uint16_t uhdata;
    uint16_t nb_rx_data;
 ...
 */
static void
UART_RxISR_16BIT_FIFOEN(UART_HandleTypeDef *huart)
{
    uint16_t *tmp;
    uint16_t  uhMask = huart->Mask;
    uint16_t  uhdata;
