...
#    define EXTI_CONFIG_OFFSET                                                 \
        0x08u /* 0x20: offset between CPU Rising/Falling configuration         \
                 registers */
/**
 * @}
 */
 ...
 */
HAL_StatusTypeDef
HAL_EXTI_SetConfigLine(EXTI_HandleTypeDef *hexti,
 ...
 */
HAL_StatusTypeDef
HAL_EXTI_GetConfigLine(EXTI_HandleTypeDef *hexti,
                       EXTI_ConfigTypeDef *pExtiConfig)
{
    __IO uint32_t *regaddr;
    uint32_t       regval;
    ...
    if ((pExtiConfig->Line & EXTI_CONFIG) != 0x00u) {
        ...
        if ((pExtiConfig->Line & EXTI_GPIO) == EXTI_GPIO) {
            ...
            regval               = EXTI->EXTICR[linepos >> 2u];
            pExtiConfig->GPIOSel = ((regval << (EXTI_EXTICR1_EXTI1_Pos
                                                * (3uL - (linepos & 0x03u))))
                                    >> 24);
        }
    }
 ...
 */
HAL_StatusTypeDef
HAL_EXTI_RegisterCallback(EXTI_HandleTypeDef    *hexti,
