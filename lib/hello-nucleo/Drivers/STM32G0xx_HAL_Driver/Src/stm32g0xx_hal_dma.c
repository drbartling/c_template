...
#    if defined(DMA2)
    while ((hdma->DmaBaseAddress->ISR & temp) == 0U) {
        if ((hdma->DmaBaseAddress->ISR
             & (DMA_FLAG_TE1 << (hdma->ChannelIndex & 0x1CU)))
            != 0U) {
...
#    else
    while (0U == __HAL_DMA_GET_FLAG(hdma, temp)) {
        if (0U
            != __HAL_DMA_GET_FLAG(
                hdma, (DMA_FLAG_TE1 << (hdma->ChannelIndex & 0x1CU)))) {
            /* When a DMA transfer error occurs */
            /* A hardware clear of its EN bits is performed */
            /* Clear all flags */
            __HAL_DMA_CLEAR_FLAG(
                hdma, ((DMA_FLAG_GI1) << (hdma->ChannelIndex & 0x1CU)));
 ...
 */
uint32_t
HAL_DMA_GetError(DMA_HandleTypeDef *hdma)
{
 ...
 */
static void
DMA_SetConfig(DMA_HandleTypeDef *hdma, uint32_t SrcAddress, uint32_t DstAddress,
              uint32_t DataLength)
{
    /* Clear the DMAMUX synchro overrun flag */
    hdma->DMAmuxChannelStatus->CFR = hdma->DMAmuxChannelStatusMask;
 ...
 */
static void
DMA_CalcDMAMUXChannelBaseAndMask(DMA_HandleTypeDef *hdma)
{
...
#    if defined(DMA2)
    ...
    if ((uint32_t)hdma->Instance < (uint32_t)DMA2_Channel1) {
         ...
         */
        channel_number =
            (((((uint32_t)hdma->Instance & 0xFFU) - 8U) / 20U) + 7U);
    }
#    else
    /* Associate a DMA Channel to a DMAMUX channel */
    hdma->DMAmuxChannel =
...
static void
DMA_CalcDMAMUXRequestGenBaseAndMask(DMA_HandleTypeDef *hdma)
{
