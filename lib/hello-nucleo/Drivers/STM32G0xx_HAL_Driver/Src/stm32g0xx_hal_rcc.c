...
#    define RCC_PLL_OSCSOURCE_CONFIG(__HAL_RCC_PLLSOURCE__)                    \
        (MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLSRC,                          \
                    (uint32_t)(__HAL_RCC_PLLSOURCE__)))
/**
 * @}
 */
        ...
        if (((temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_PLLCLK)
            ...
            /* Otherwise, just the calibration is allowed */
                ...
                if (temp_sysclksrc == RCC_SYSCLKSOURCE_STATUS_HSI) {
                    ...
                    SystemCoreClock =
                        (HSI_VALUE
                         / (1UL << ((READ_BIT(RCC->CR, RCC_CR_HSIDIV))
                                    >> RCC_CR_HSIDIV_Pos)));
                }
...
#    else  /* !RCC_PLLQ_SUPPORT */
                __HAL_RCC_PLL_CONFIG(
                    RCC_OscInitStruct->PLL.PLLSource,
                    RCC_OscInitStruct->PLL.PLLM, RCC_OscInitStruct->PLL.PLLN,
                    RCC_OscInitStruct->PLL.PLLP, RCC_OscInitStruct->PLL.PLLR);
#    endif /* RCC_PLLQ_SUPPORT */
 ...
 *            @arg @ref RCC_MCO1SOURCE_HSE      HSE clock selected as MCO
 * sourcee
 ...
 */
void
HAL_RCC_MCOConfig(uint32_t RCC_MCOx, uint32_t RCC_MCOSource,
                  uint32_t RCC_MCODiv)
{
...
HAL_RCC_GetSysClockFreq(void)
{
    ...
    if (__HAL_RCC_GET_SYSCLK_SOURCE() == RCC_SYSCLKSOURCE_STATUS_HSI) {
    ...
    } else if (__HAL_RCC_GET_SYSCLK_SOURCE()
        ...
        */
        ...
        pllr =
            (((RCC->PLLCFGR & RCC_PLLCFGR_PLLR) >> RCC_PLLCFGR_PLLR_Pos) + 1U);
 ...
 */
void
HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct)
{
    /* Check the parameters */
 ...
 */
void
HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *RCC_ClkInitStruct,
                       uint32_t           *pFLatency)
{
    /* Check the parameters */
